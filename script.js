...
teamGen.addEventListener("click", (e) => {
  ...
  while (namePush.length < nameRest.length)
    namePush.push(
      nameRest.splice(Math.floor(Math.random() * nameRest.length), 1)[0]
    );

  //afficher nos deux tableaux pour l utilisateur

  var newGroupe1 = document.createElement("p");
  document.getElementById("groupe1").appendChild(newGroupe1).innerHTML =
    nameRest;
  var newGroupe2 = document.createElement("p");
  document.getElementById("groupe2").appendChild(newGroupe2).innerHTML =
    namePush;
});
